...
function VisaForm() {
  ...
  // const onSubmit = data => console.log(data);
	...
	const session = useMemo(() => dataSession, [dataSession])

	const [userData, setUserData] = useState(null)

	useEffect(() => {
		setValue('jenis_kelamin', 'L')
		setValue('jenis_permohonan', 1)
		setValue('lama_permohonan', 1)
		setValue('status_sipil', 1)
		setValue('status_usaha', 1)
		setValue('kategori_penjamin', 1)
		setValue('bidang_pekerjaan', 1)
	}, [])
	...
	useEffect(() => {
		...
		async function getUserData() {
			try {
				const url = session.user.role_id == 4 ? '/api/student?user_id=' + session.user.userID :
										session.user.role_id == 3 ? '/api/teacher?user_id=' + session.user.userID :
										null
				if(url) {
					const { data } = await axios.get(url)	
          console.log(`🚀 ~ file: index.js ~ line 226 ~ getUserData ~ data`, data)
					if(!data)
						return
					...
					const { first_name, middle_name, last_name, gender, place_of_birth, date_of_birth, identity_id, identity_type_id, expiredVisa, nationality } = data.data.user_info
					setValue('nama_lengkap',`${first_name} ${middle_name ? middle_name + ' ' : '' }${last_name}`)
					setValue('jenis_kelamin', gender == "MAN" ? "L" : "P")
					setValue('tempat_lahir', place_of_birth)
					setValue('tanggal_lahir', new Date(date_of_birth))
					setValue('no_passport', identity_type_id == 2 ? identity_id : '')
					setValue('berlaku_sampai', expiredVisa ? new Date(expiredVisa) : '')
					setValue('kewarganegaraan', nationality)
				}
			} catch (error) {
				if(error.response)	
	...
	async function onSubmit(data) {
		try {
			let prepareData = structuredClone(data)
			if(session.user.role_id != 1)
				prepareData.user_id = session.user.userID
			await axios.post('/api/visa-form',[prepareData])	
						...
						> */}
							...
							>
								...
								>
									...
									/>
									<ControllerTextField control={control} name="kewarganegaraan" label="KEWARGANEGARAAN" />
